...
pour lier les nom des espèces aux echantillons qui leurs appartiennent
...
data2_short[data2_short$espece == especes[1],c("motu","count")] %>% right_join(select(motus2_ref[motus2_ref$count > 3000,], id), by = c("motu" = "id")) -> sub_vect_count
sub_vect_count
```
...
for (e in especes[1:9]) {
  ...
  df_spectre_hill <- rbind(df_spectre_hill, d %>% group_by(q) %>% mutate(Qd =  spectre_hill(val = val, q = q))) # spectre de hill pour tout les echantillons aprtenant à une même espèce
}
...
```{r}
# data2[data2$espece == especes[1],]

data2 %>% group_by(espece,motu) %>% summarise(count = sum(count)) -> data2_short
View(data2_short)
data2_short[data2_short$espece == especes[1],]
```
...
```{r}
motus2[,c("id","count")]
data2_short[data2_short$espece == especes[1],c("motu","count")] #permet de ne selectionner que les deux colonnes de data2 short pour l'espèce 1

data2_short[data2_short$espece == especes[1],c("motu","count")] %>% right_join(select(motus2_ref[motus2_ref$count > 3000,], id), by = c("motu" = "id")) -> sub_vect_count
sub_vect_count


nrow(motus2_ref[motus2_ref$count > 3000,])
nrow(mat_proba2)
```
