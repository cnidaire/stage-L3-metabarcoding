...
ajouter dans ref et dans ref + mutant en utilisant data2_short

pour cela, il faudra créer une nouvelle colonne dans ref à côté de data2 short -> dans ref

```{r}
df_spectre_hill = data.frame()

d = data.frame(espece = especes [1], donnee = "Rectifié" ,q = q_values)
data2_short[data2_short$espece == especes [1], c("motu","count")] %>% right_join(select(motus2_ref[motus2_ref$count > 3000,], id), by = c("motu" = "id")) -> sub_vect_count # rédaction d'un vecteur avec les counts et de la dimension de la matrice de proba
sub_vect_count[is.na(sub_vect_count$count),]$count <- 0 # remplacement des NA par des 0
sub_count_th <- t(t(sub_vect_count$count) %*% mat_proba2)# produit matriciel entre count et mat proba
...
dans l'autre il faudra se débrouiller avec le sub_count_th calculé précédemment dans la boucle for : rajouter une colonne dans la ref et ne sélectionner que les motus appartenant à la ref comme ce qui a été fait précédemment dans la base de données en entier.
