...
matrice_probabilite <- function(matrice_distance, base_donnee, base_donnee_ref){
  matrice_proba <- matrix(data = NA, nrow = nrow(matrice_distance), ncol = ncol(matrice_distance))
  for (i in 1:nrow(matrice_distance)) {
    if (base_donnee_ref$dans_ref[i] == T) {
      matrice_proba[i,] <- 1 # n'est lié à rien d'autre
    }else {
      if (1 %in% matrice_distance[i,1:(i-1)]) { # si il y a au moins un motu à 1 de distance génétique
        a <- which(matrice_distance[i,1:(i-1)] == 1) # on marque lesquels sont à un de distance
        if (length(a) == 1) {
          ...
          matrice_proba[i,] <- 0
          matrice_proba[i,a] <- (motus$count[a]/sum(motus$count[a])) # lié aux deux mais perte de l'info de la répartition entre les deux
        }
      }else{
        matrice_proba[i,] <- 1 # définition d'une probable nouvelle espèce
      }
    }
  }
  return(matrice_proba)
}
matrice_proba <- matrice_probabilite(mat_dist2, "base de donnée", motus2_ref)
View(matrice_proba)
```
...
<!-- especes = especes[!is.na(especes)] -->
<!-- # especes[1] -->
<!-- summary(data[data$espece == especes[1],]$count) -->
<!-- View(data[data$espece == especes[1],]) -->
<!-- # system.time({ -->
<!-- # mat_dist <-matrice_distance(data[data$espece == especes[1],], 300) -->
<!-- # }) -->
<!-- nrow(mat_dist) -->
<!-- mat_dist2 <- matrice_distance(motus2,3000) -->
<!-- View(mat_dist2) -->
...
## Regrouper ensemble les motus variants avec le motu originel afin de sommer le nombre de reads

## Tracer le spectre de Hill pour une espece
