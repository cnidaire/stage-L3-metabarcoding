...
```{r}
x <- sapply(motus2$sequence[1:nrow(motus2)], lcs_score, motus2$sequence[1:nrow(motus2)], similarity_mode = "distance")
matrice_distance2 <- matrix(unlist(x), nrow = length(motus2$sequence[1:nrow(motus2)]), ncol = length(motus2$sequence[1:nrow(motus2)]), byrow = T)
...
```{r}
motus2[ , "dans_ref"] <- F # créer une nouvelle colonne dans motus et l'initialiser avec des FALSE
for (c in 1:nrow(motus2)) {
  if (motus2$sequence[c] %in% reference$Sequence.ID) {
    motus2$dans_ref[c] <- T
  }
} 
```
...
```{r}
matrice_adj2 <- matrix(data = NA, nrow = nrow(motus2), ncol = nrow(motus2))

couleur <- NULL
for (i in 1:nrow(motus2)) {
  if (motus2$dans_ref[i] == T) {
...
taille_vertex <- 3*log10(motus2$count)

graph_liens <- graph_from_adjacency_matrix(matrice_adj2[1:nrow(motus2),1:nrow(motus2)])
V(graph_liens)$color <- couleur
V(graph_liens)$size <- taille_vertex
...
mettre un filtre pour retirer les motus avec moins de 2000
