...
hill_estimation <- data.frame(q=nombre_q, Qd=sapply(nombre_q,spectre_hill, count_espece[count_espece$count_estime != 0,]$count_estime))
...
```{r}
Graph <- ggplot(data = hill_estimation, aes(x = q, y = log10(Qd))) +
  # On trace la ligne
  geom_line() +
...
```{r}
ps <- ((count_espece[count_espece$count_estime != 0,]$count_estime)/ (sum(count_espece[count_espece$count_estime != 0,]$count_estime)))
ps
```
