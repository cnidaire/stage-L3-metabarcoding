...
# 1) Importantion des packages necessaires et des données
...
# 2) Prise en main et visualisation des données
...
nrow(data_count)
data_count # permet de re-aggreger les données de data
...
# # q
# # ps
# spectre_hill(c(0,1),"~04A_1")
```
problème dans les valeurs affichées.
il y a un problème dans la fonction pour essayer de trouver l'erreur, je vais refaire une autre fonction en utilisant if et else avec un sapply pour pouvoir implémenter dans la fonction les différentes valeurs de q au lieu du ifelse.
...
df <- NULL
for (ech in unique(data$sample)[1:10] ) {
  df <- rbind(df, data.frame(sample=ech,
                             q=nombre_q,
                             Qd=sapply(nombre_q,spectre_hill,ech)))
}
df # ici, j'ai choisi de le faire sur les 10 premier échantillons
```
...
# dfacet <- data.frame(unique(df$sample),1:4)
unique(df$sample)
dfacet <- data.frame(sample = unique(df$sample), f = rep(1:4, floor(((length(unique(df$sample)))/4)+1)[1:length(unique(df$sample))]))

# ne fonctionne pas

df <- left_join(df,dfacet, by = sample)
# essai de noter sur le dataframe des numéros qui nous permettrons par la suit de pouvoir fractionner en plusieurs graphiques à l'aide de facet_wrap
...
pour pouvoir afficher plusieurs graphs les uns à côté des autres, quelque chose existe, c'est peut être facet_grid/ facet wrap mais je ne suis pas sûr du tout

# 4 ) Tracé du spectre de Hill pour les différentes espèces (le lien entre espèce et échantillon est dans le fichier samples). 
