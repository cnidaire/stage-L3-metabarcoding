...
objectifs: calculer une matrice de distance
tracer un autre graph

mise en place d'un jeu de données filtrées motus2bis avec un filtre pour les motu avec un nombre de reads inférieur à 2000
```{r}
motus2bis <- motus2[motus2$count >2000,]
motus2bis
x <- sapply(motus2bis$sequence[1:nrow(motus2bis)], lcs_score, motus2bis$sequence[1:nrow(motus2bis)], similarity_mode = "distance")
matrice_distance2 <- matrix(unlist(x), nrow = length(motus2bis$sequence[1:nrow(motus2bis)]), ncol = length(motus2bis$sequence[1:nrow(motus2bis)]), byrow = T)
# View(matrice_distance2)
```

(ce calcul a mis 2min20sec à être fait)


mettre dans motus les séquences appartenant à la ref

```{r}
motus2bis[ , "dans_ref"] <- F # créer une nouvelle colonne dans motus et l'initialiser avec des FALSE
for (c in 1:nrow(motus2bis)) {
  if (motus2bis$sequence[c] %in% reference$Sequence.ID) {
    motus2bis$dans_ref[c] <- T
  }
} 
```
...
# Retirer ceux qui n'ont pas d’arêtes (ce qui ne pointent que vers eux et vers qui personne ne pointe).

regarder dans la matrice de proba si le motu a une proba d'appartenance à lui même de 1 et si c'est la cas, si il est le seul dans sa colonne à être renvoyé vers lui
...
length(which(matrice_adj2 [,1] == 1)) == 78
...
```{r}
motu_supr <- vector()
for (i in 1:nrow(matrice_adj2)) {
  if (length(which(matrice_adj2[,i]))==0) {
      motu_supr <- c(motu_supr,i)
  }
}

matrice_adj2bis <- matrice_adj2
matrice_adj2bis[motu_supr,] <-NULL
matrice_adj2bis[,motu_supr] <-NULL
